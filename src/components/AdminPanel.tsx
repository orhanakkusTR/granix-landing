...
import { X, Download, Calendar, Mail, MapPin, FileText, Package, Lock, Trash2 } from 'lucide-react';
...
export default function AdminPanel() {
  ...
  const handleDeleteSelected = async () => {
    ...
    const confirmed = confirm(`${selectedIds.size} lead'i silmek istediğinizden emin misiniz?`);
    if (!confirmed) return;
  ...
  const exportToCSV = () => {
    const headers = ['Date', 'Name', 'Email', 'Phone', 'Address', 'ZIP', 'Material', 'Project Type', 'Status', 'Comments'];
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {submissions.map((submission) => (
                  <tr key={submission.id} className={`hover:opacity-90 ${getStatusColor(submission.status)}`}>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm">
                      <select
                        ...
                        className="block w-full px-2 py-1.5 border border-gray-300 rounded-md shadow-sm focus:ring-amber-500 focus:border-amber-500 text-xs font-medium"
                      >
